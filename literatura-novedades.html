...
<head>
    ...
    <title>Novedades de Literatura - BAMARTE</title>
        ...
        <main class="py-16">
            <section>
                <div class="text-center mb-12">
                    <h2 class="text-3xl font-bold text-emerald-500">Novedades de Literatura</h2>
                    <p class="text-lg text-gray-600 mt-2">Todas las noticias y artículos del mundo de la literatura, ordenadas por fecha.</p>
                </div>
    ...
    <script type="module">
        ...
        function createNewsCard(newsItem) {
            ...
            <a href="news-detail.html?id=${newsItem.id}" class="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 flex flex-col group">
                <div class="relative">
                    <img src="${newsItem.mainImageUrl || 'https://placehold.co/400x250/d1fae5/10b981?text=BAMARTE'}" alt="${newsItem.title}" class="w-full h-56 object-cover" onerror="this.onerror=null;this.src='https://placehold.co/400x250/d1fae5/10b981?text=BAMARTE';">
                </div>
                <div class="p-6 flex flex-col flex-grow">
                    <p class="text-xs text-emerald-600 font-bold mb-2 uppercase tracking-widest">${dateString}</p>
                    <h4 class="text-xl font-black mb-2 uppercase group-hover:text-emerald-600 transition-colors leading-tight">${newsItem.title}</h4>
                    <p class="text-gray-600 text-sm flex-grow font-medium">${(newsItem.content || '').substring(0, 150)}...</p>
                </div>
            </a>`;
        }

        async function fetchAndDisplayAllLiteratureNews() {
            try {
                ...
                if (querySnapshot.empty) {
                    newsContainer.innerHTML = `<p class="col-span-full text-center text-gray-500 font-bold uppercase tracking-widest py-10">No hay noticias de literatura en este momento.</p>`;
                    return;
                }
                ...
                console.error("Error al obtener las noticias de literatura: ", error);
                newsLoader.style.display = 'none';
        ...
        fetchAndDisplayAllLiteratureNews();
    </script>
