<x-app-layout>
    ...
    <x-breadcrumb :items="array_filter([
        'Artes Visuales' => $subCat ? route('arte') : null,
        $subCat => null
    ])"/>
    <main>
        <!-- 1. Category Header -->
        <header class="py-6" style="background-color: var(--color-visuales-light); border-bottom: 3px solid var(--color-visuales);">
        ...
        <section class="pt-10 pb-12 bg-white">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="rounded-2xl p-6 shadow-[0_2px_16px_rgba(0,0,0,0.06)]" style="background: linear-gradient(to top, color-mix(in srgb, var(--color-visuales) 12%, white), color-mix(in srgb, var(--color-visuales) 4%, white)); border: 1px solid color-mix(in srgb, var(--color-visuales) 25%, white);">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <!-- Carrusel grande (Izquierda) -->
                    <div class="lg:col-span-2 flex">
                        <div class="swiper carousel-visuales rounded-lg overflow-hidden relative w-full">
                                ...
                                <div class="swiper-slide">
                                    <a href="{{ $linkDe($item) }}" class="block relative h-[480px]">
                                        ...
                                        <div class="absolute bottom-0 left-0 p-8 text-white">
                                            <span class="text-sm font-bold uppercase tracking-wider" style="color: #fff; background: var(--color-visuales); padding: 3px 10px; border-radius: 9999px; display: inline-block;">{{ $item->category }}@if($item->subCategory) &middot; {{ $item->subCategory }}@endif</span>
                    ...
                    <div class="hidden sm:flex flex-col" id="lista-visuales">
                            ...
                            <div class="lista-track flex flex-col gap-4 transition-transform duration-300">
                                ...
                                <a href="{{ $linkDe($item) }}" class="flex gap-3 p-3 border border-[var(--border-color)] rounded-lg bg-white hover:bg-gray-50 transition flex-shrink-0">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <span class="text-xs font-bold uppercase" style="color: var(--color-visuales);">{{ $item->category }}</span>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                if (document.querySelector('.carousel-visuales')) {
                    new Swiper('.carousel-visuales', {
                        loop: true,
                        autoplay: { delay: 3800, disableOnInteraction: false },
                        pagination: { el: '.carousel-visuales .swiper-pagination', clickable: true },
                        navigation: { nextEl: '.carousel-visuales .swiper-button-next', prevEl: '.carousel-visuales .swiper-button-prev' },
                    });
                }

            // Scroll de la lista lateral con flechas
            (function() {
                const cont = document.getElementById('lista-visuales');
                if (!cont) return;
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="mt-6 flex justify-center items-center gap-2 md:hidden" x-show="totalPaginas > 1">
                    ...
                    <template x-for="p in totalPaginas" :key="p">
                        <button @click="pagina = p" :class="pagina === p ? 'text-white' : 'text-gray-600 border border-gray-300'" :style="pagina === p ? 'background-color: var(--color-visuales)' : ''" class="w-9 h-9 flex items-center justify-center rounded-full text-sm font-bold" x-text="p"></button>
                    </template>
