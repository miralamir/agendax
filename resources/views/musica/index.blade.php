<x-app-layout>
    ...
    <x-breadcrumb :items="array_filter([
        'Música' => $subCat ? route('musica') : null,
        $subCat => null
    ])"/>
    <main>
        <!-- 1. Category Header -->
        <header class="py-6" style="background-color: var(--color-musica-light); border-bottom: 3px solid var(--color-musica);">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <h1 class="text-6xl font-black" style="color: var(--color-musica);">Música</h1>
                <div class="mt-4">
                <div class="flex flex-wrap items-center gap-2">
                    <a href="{{ request()->fullUrlWithoutQuery('sub') }}" class="px-4 py-2 text-sm font-bold rounded-full {{ !request('sub') ? 'text-white' : 'border' }}" style="background-color: {{ !request('sub') ? 'var(--color-musica)' : 'transparent' }}; color: {{ !request('sub') ? 'white' : 'var(--color-musica)' }}; border-color: {{ !request('sub') ? 'none' : 'var(--color-musica)' }};">Todos</a>
                    <a href="{{ request()->fullUrlWithQuery(['sub' => 'Agenda']) }}" class="px-4 py-2 text-sm font-bold rounded-full border {{ request('sub') == 'Agenda' ? 'text-white' : '' }}" style="background-color: {{ request('sub') == 'Agenda' ? 'var(--color-musica)' : 'transparent' }}; color: {{ request('sub') == 'Agenda' ? 'white' : 'var(--color-musica)' }}; border-color: var(--color-musica);">Agenda</a>
                    <a href="{{ request()->fullUrlWithQuery(['sub' => 'Lanzamientos']) }}" class="px-4 py-2 text-sm font-bold rounded-full border {{ request('sub') == 'Lanzamientos' ? 'text-white' : '' }}" style="background-color: {{ request('sub') == 'Lanzamientos' ? 'var(--color-musica)' : 'transparent' }}; color: {{ request('sub') == 'Lanzamientos' ? 'white' : 'var(--color-musica)' }}; border-color: var(--color-musica);">Lanzamientos</a>
                    <a href="{{ request()->fullUrlWithQuery(['sub' => 'Festivales']) }}" class="px-4 py-2 text-sm font-bold rounded-full border {{ request('sub') == 'Festivales' ? 'text-white' : '' }}" style="background-color: {{ request('sub') == 'Festivales' ? 'var(--color-musica)' : 'transparent' }}; color: {{ request('sub') == 'Festivales' ? 'white' : 'var(--color-musica)' }}; border-color: var(--color-musica);">Festivales</a>
                    <a href="{{ request()->fullUrlWithQuery(['sub' => 'Novedades']) }}" class="px-4 py-2 text-sm font-bold rounded-full border {{ request('sub') == 'Novedades' ? 'text-white' : '' }}" style="background-color: {{ request('sub') == 'Novedades' ? 'var(--color-musica)' : 'transparent' }}; color: {{ request('sub') == 'Novedades' ? 'white' : 'var(--color-musica)' }}; border-color: var(--color-musica);">Novedades</a>
                    <form method="GET" class="ml-auto flex items-center">
                        @if($subCat)<input type="hidden" name="sub" value="{{ $subCat }}">@endif
                        <input type="text" name="q" value="{{ request('q') }}" placeholder="Buscar en Música..."
        ...
        <section class="pt-10 pb-12 bg-white">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="rounded-2xl p-6 shadow-[0_2px_16px_rgba(0,0,0,0.06)]" style="background: linear-gradient(to top, color-mix(in srgb, var(--color-musica) 12%, white), color-mix(in srgb, var(--color-musica) 4%, white)); border: 1px solid color-mix(in srgb, var(--color-musica) 25%, white);">
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div class="hidden sm:flex flex-col" id="lista-musica">
                            ...
                            <div class="lista-track flex flex-col gap-4 transition-transform duration-300">
                                ...
                                <a href="{{ $linkDe($item) }}" class="flex gap-3 p-3 border border-[var(--border-color)] rounded-lg bg-white hover:bg-gray-50 transition flex-shrink-0">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <span class="text-xs font-bold uppercase" style="color: var(--color-musica);">{{ $item->category }}</span>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                if (document.querySelector('.carousel-musica')) {
                    new Swiper('.carousel-musica', {
                        loop: true,
                        autoplay: { delay: 3800, disableOnInteraction: false },
                        pagination: { el: '.carousel-musica .swiper-pagination', clickable: true },
                        navigation: { nextEl: '.carousel-musica .swiper-button-next', prevEl: '.carousel-musica .swiper-button-prev' },
                    });
                }
        ...
        <section class="pt-4 pb-16 bg-gray-50">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center mb-8">
                    ...
                    <a href="#" class="text-sm font-bold" style="color: var(--color-musica);">Ver todos &rarr;</a>
                </div>
                ...
                <div class="mt-6 flex justify-center items-center gap-2 md:hidden" x-show="totalPaginas > 1">
                    ...
                    <template x-for="p in totalPaginas" :key="p">
                        <button @click="pagina = p" :class="pagina === p ? 'text-white' : 'text-gray-600 border border-gray-300'" :style="pagina === p ? 'background-color: var(--color-musica)' : ''" class="w-9 h-9 flex items-center justify-center rounded-full text-sm font-bold" x-text="p"></button>
                    </template>
