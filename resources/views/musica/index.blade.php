<x-app-layout>
    <main>
        <!-- 1. Category Header -->
        <header class="py-12" style="background-color: var(--color-musica-light); border-bottom: 3px solid var(--color-musica);">
                    ...
                    @foreach($latestItems as $item)
                        ...
                        <a href="{{ $link }}" class="group block bg-white rounded-lg border border-[var(--border-color)] hover:translate-y-[-2px] hover:shadow-[0_4px_12px_rgba(0,0,0,0.09)] transition-all duration-300 overflow-hidden">
                            <div class="relative h-48 overflow-hidden">
                                @if($imageUrl)
                                    <img src="{{ Storage::url($imageUrl) }}" alt="{{ $item->title }}" class="w-full h-full object-cover">
                                @else
