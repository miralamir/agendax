...
<head>
    ...
    <title>Agenda de Literatura - BAMARTE</title>
    ...
    <style>
        ...
        .loader {
            ...
            border-top: 4px solid #10b981; /* Emerald 500 para Literatura */
            border-radius: 50%;
            width: 40px;
            height: 40px;
...
<body class="text-gray-800">
    ...
    <div class="container mx-auto px-4">
        ...
        <!-- Contenido Principal de la Página de Literatura -->
        <main class="py-16">
            <div class="text-center mb-12">
                <h1 class="text-4xl md:text-5xl font-black text-emerald-500">Agenda de Literatura</h1>
                <p class="text-lg text-gray-600 mt-2">Presentaciones de libros, charlas, ferias y eventos literarios.</p>
            </div>
    ...
    <script type="module">
        ...
        async function fetchAndDisplayLiteraturaEvents() {
            try {
                const eventsRef = collection(db, "events");
                const q = query(
                    eventsRef, 
                    where("isPublished", "==", true),
                    where("category", "==", "literatura")
                );
                ...
                upcomingEvents.forEach((event) => {
                    ...
                    const eventCard = `
                        <a href="event-detail.html?id=${event.id}" class="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 flex flex-col group">
                            <div class="relative">
                                <img src="${event.mainImageUrl || 'https://placehold.co/400x250/e9d5ff/10b981?text=BAMARTE'}" alt="${event.title}" class="w-full h-56 object-cover" onerror="this.onerror=null;this.src='https://placehold.co/400x250/e9d5ff/10b981?text=BAMARTE';">
                                <div class="absolute top-2 right-2 bg-white bg-opacity-90 text-emerald-700 font-black text-xs px-3 py-1 rounded-full shadow-sm uppercase tracking-widest">${dateString}</div>
                            </div>
                            <div class="p-6 flex flex-col flex-grow">
                                <h4 class="text-xl font-black mb-2 uppercase group-hover:text-emerald-600 transition-colors leading-tight">${event.title}</h4>
                ...
                console.error("Error al obtener los eventos de literatura: ", error);
                loaderContainer.style.display = 'none';
                eventsContainer.innerHTML = `<p class="col-span-full text-center text-red-500 p-4 bg-red-100 rounded-lg font-bold">Ocurrió un error al cargar los eventos. Es posible que necesites crear un índice en Firestore. Revisa la consola del navegador para ver el enlace de creación.</p>`;
            }
        }

        fetchAndDisplayLiteraturaEvents();
    </script>
