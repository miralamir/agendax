...
<body class="bg-gray-100 text-gray-800 pb-20">
            ...
            <div class="p-4 md:p-8 max-w-5xl mx-auto">
                <form id="edit-event-form" class="space-y-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select name="subCategory" id="form-subCategory" class="w-full px-4 py-2 border rounded-lg bg-white outline-none font-bold">
                                    <option value="agenda">Agenda General</option>
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
                        ...
                        <p class="text-xs text-slate-500 mb-6 font-bold">Rellena solo los campos que apliquen a tu evento.</p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div><label class="block text-sm font-bold text-purple-700 mb-2">Latitud</label><input type="text" name="latitude" id="form-latitude" placeholder="-34.6036" class="w-full px-4 py-2 border border-purple-200 rounded-lg font-bold bg-purple-50"></div>
                                <div><label class="block text-sm font-bold text-purple-700 mb-2">Longitud</label><input type="text" name="longitude" id="form-longitude" placeholder="-58.3694" class="w-full px-4 py-2 border border-purple-200 rounded-lg font-bold bg-purple-50"></div>
                            </div>
                        </div>
                    </div>
    ...
    <script type="module">
        ...
        // IMPORTAMOS GeoPoint NATIVO DE FIREBASE
        import { getFirestore, doc, getDoc, updateDoc, collection, getDocs, Timestamp, GeoPoint } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";
        ...
        async function loadEventData(id) {
            try {
                ...
                // LÓGICA INFALIBLE PARA LEER COORDENADAS (Prioriza el GeoPoint si los textos están vacíos)
                let loadedLat = data.latitude || '';
                let loadedLng = data.longitude || '';
                if (data.locationGeoPoint && (!loadedLat || !loadedLng)) {
                    loadedLat = data.locationGeoPoint.latitude;
                    loadedLng = data.locationGeoPoint.longitude;
                }
                document.getElementById('form-latitude').value = loadedLat;
                document.getElementById('form-longitude').value = loadedLng;
        ...
        document.getElementById('edit-event-form').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                overlayText.textContent = "Procesando Coordenadas exactas...";
                
                // --- CREACIÓN DEL OBJETO GEOPOINT NATIVO ---
                let latValue = formData.get('latitude') ? formData.get('latitude').trim().replace(',', '.') : '';
                let lngValue = formData.get('longitude') ? formData.get('longitude').trim().replace(',', '.') : '';
                
                let locationGeoPointObj = null;
                if (latValue !== '' && lngValue !== '') {
                    const parsedLat = parseFloat(latValue);
                    const parsedLng = parseFloat(lngValue);
                    if (!isNaN(parsedLat) && !isNaN(parsedLng)) {
                        // AQUÍ ESTÁ LA MAGIA: Clase nativa GeoPoint
                        locationGeoPointObj = new GeoPoint(parsedLat, parsedLng);
                    }
                }
                ...
                await updateDoc(doc(db, "events", eventId), {
                    ...
                    latitude: latValue,
                    longitude: lngValue,
                    locationGeoPoint: locationGeoPointObj, 
