...
<head>
    ...
    <title>Festivales de Música - BAMARTE</title>
...
<body class="text-gray-800">
    ...
    <div class="container mx-auto px-4">
        ...
        <!-- Contenido Principal de la Página de Festivales de Música -->
        <main class="py-16">
            <section class="mb-16">
                <div class="text-center mb-12">
                    <h2 class="text-3xl font-bold text-orange-500">Festivales de Música</h2>
                    <p class="text-lg text-gray-600 mt-2">Una selección de los festivales de música más importantes de la ciudad.</p>
                </div>
    ...
    <script type="module">
        ...
        function createEventCard(event) {
            ...
            return `
                <a href="event-detail.html?id=${event.id}" class="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 flex flex-col group">
                    <div class="relative">
                        <img src="${event.mainImageUrl || 'https://placehold.co/400x250/ffedd5/fb923c?text=BAMARTE'}" alt="${event.title}" class="w-full h-56 object-cover" onerror="this.onerror=null;this.src='https://placehold.co/400x250/ffedd5/fb923c?text=BAMARTE';">
                        <div class="absolute top-2 right-2 bg-white bg-opacity-90 text-orange-700 font-black text-xs px-3 py-1 rounded-full shadow-sm uppercase tracking-widest">${dateString}</div>
                    </div>
                    <div class="p-6 flex flex-col flex-grow">
                        <h4 class="text-xl font-black mb-2 uppercase group-hover:text-orange-600 transition-colors leading-tight">${event.title}</h4>
        ...
        async function fetchAndDisplayMusicFestivals() {
            try {
                ...
                if (upcomingEvents.length === 0) {
                    eventsContainer.innerHTML = `<p class="col-span-full text-center text-gray-500 font-bold uppercase tracking-widest py-10">No hay festivales de música en la agenda en este momento.</p>`;
                } else {
                ...
                console.error("Error al obtener los festivales de música: ", error);
                eventsLoader.style.display = 'none';
                eventsContainer.innerHTML = `<p class="col-span-full text-center text-red-500 p-4 bg-red-100 rounded-lg font-bold">Ocurrió un error al cargar los festivales. Es posible que necesites crear un índice compuesto en Firestore.</p>`;
            }
        }

        fetchAndDisplayMusicFestivals();
    </script>
