...
<head>
    ...
    <title>Novedades de Teatro - BAMARTE</title>
        ...
        <main class="py-16">
            <section>
                <div class="text-center mb-12">
                    <h2 class="text-3xl font-bold text-pink-500">Novedades de Teatro</h2>
                    <p class="text-lg text-gray-600 mt-2">Todas las noticias y artículos del mundo del teatro, ordenadas por fecha.</p>
                </div>
    ...
    <script type="module">
        ...
        function createNewsCard(newsItem) {
            ...
            <a href="news-detail.html?id=${newsItem.id}" class="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 flex flex-col group">
                <div class="relative">
                    <img src="${newsItem.mainImageUrl || 'https://placehold.co/400x250/fbcfe8/ec4899?text=BAMARTE'}" alt="${newsItem.title}" class="w-full h-56 object-cover" onerror="this.onerror=null;this.src='https://placehold.co/400x250/fbcfe8/ec4899?text=BAMARTE';">
                </div>
                <div class="p-6 flex flex-col flex-grow">
                    <p class="text-xs text-pink-600 font-bold mb-2 uppercase tracking-widest">${dateString}</p>
                    <h4 class="text-xl font-black mb-2 uppercase group-hover:text-pink-600 transition-colors leading-tight">${newsItem.title}</h4>
                    <p class="text-gray-600 text-sm flex-grow font-medium">${(newsItem.content || '').substring(0, 150)}...</p>
                </div>
            </a>`;
        }

        async function fetchAndDisplayAllTheaterNews() {
            try {
                ...
                if (querySnapshot.empty) {
                    newsContainer.innerHTML = `<p class="col-span-full text-center text-gray-500 font-bold uppercase tracking-widest py-10">No hay noticias de teatro en este momento.</p>`;
                    return;
                }
                ...
                console.error("Error al obtener las noticias de teatro: ", error);
                newsLoader.style.display = 'none';
