...
<head>
    ...
    <title>Agenda de Artes Visuales - BAMARTE</title>
...
<body class="text-gray-800">
    ...
    <div class="container mx-auto px-4">
        ...
        <header class="py-6">
            <nav class="flex justify-between items-center">
                ...
                <ul class="hidden md:flex items-center space-x-8 font-bold text-sm">
                    ...
                    <li><a href="musica.html" class="uppercase text-orange-500 hover:opacity-75 transition-opacity duration-300">Música</a></li>
                    <li><a href="teatro.html" class="uppercase text-pink-500 hover:opacity-75 transition-opacity duration-300">Teatro</a></li>
                    <li><a href="cine.html" class="uppercase text-blue-500 hover:opacity-75 transition-opacity duration-300">Cine</a></li>
                    <li><a href="literatura.html" class="uppercase text-emerald-500 hover:opacity-75 transition-opacity duration-300">Literatura</a></li>
                </ul>
        ...
        <!-- Contenido Principal de la Página de Arte -->
        <main class="py-16">
            ...
            <section class="mb-16">
                ...
                <div id="featured-events-container" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    <div id="featured-events-loader" class="col-span-full flex justify-center items-center h-40">
            ...
            <section>
                ...
                <div id="featured-news-container" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    <div id="featured-news-loader" class="col-span-full flex justify-center items-center h-40">
                        <div class="loader"></div>
                    </div>
                </div>
            </section>
    ...
    <script type="module">
        ...
        const featuredEventsLoader = document.getElementById('featured-events-loader');
        const featuredNewsContainer = document.getElementById('featured-news-container');
        const featuredNewsLoader = document.getElementById('featured-news-loader');

        // Función inteligente para crear la tarjeta de Evento
        ...
        // Función inteligente para crear la tarjeta de Noticias
        function createNewsCard(newsItem) {
            let dateString = '';
            // Verificamos de forma segura que la fecha exista y sea válida
            ...
            return `
                <a href="#" class="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 flex flex-col group">
                    <div class="relative">
                        <img src="${newsItem.mainImageUrl || 'https://placehold.co/400x250/e9d5ff/a855f7?text=BAMARTE'}" alt="${newsItem.title}" class="w-full h-56 object-cover" onerror="this.onerror=null;this.src='https://placehold.co/400x250/e9d5ff/a855f7?text=BAMARTE';">
                    </div>
                    <div class="p-6 flex flex-col flex-grow">
                        <p class="text-xs text-cyan-600 font-bold mb-2 uppercase tracking-widest">${dateString}</p>
                        <h4 class="text-xl font-black mb-2 uppercase group-hover:text-cyan-600 transition-colors leading-tight">${newsItem.title}</h4>
        ...
        // Fetch Eventos
        async function fetchAndDisplayFeaturedArtEvents() {
            try {
                ...
                featuredEventsLoader.style.display = 'none';
                ...
                // Lógica de filtrado seguro (A prueba de balas)
                ...
                if (upcomingEvents.length === 0) {
                    featuredEventsContainer.innerHTML = `<p class="col-span-full text-center text-gray-500 font-bold uppercase tracking-widest py-10">No hay eventos de artes visuales destacados en este momento.</p>`;
                } else {
                    // AQUÍ ESTABA EL ERROR REPARADO: Solo 1 bucle forEach
                    upcomingEvents.forEach((event) => {
                        featuredEventsContainer.innerHTML += createEventCard(event);
                    });
                }
            } catch (error) {
                console.error("Error al obtener los eventos destacados de arte: ", error);
                featuredEventsLoader.style.display = 'none';
                featuredEventsContainer.innerHTML = `<p class="col-span-full text-center text-red-500 p-4 bg-red-100 rounded-lg font-bold">Ocurrió un error al cargar los eventos destacados.</p>`;
            }
        }

        // Fetch Noticias
        async function fetchAndDisplayFeaturedArtNews() {
            try {
                ...
                featuredNewsLoader.style.display = 'none';
                featuredNewsContainer.innerHTML = '';

                if (querySnapshot.empty) {
                    featuredNewsContainer.innerHTML = `<p class="col-span-full text-center text-gray-500 font-bold uppercase tracking-widest py-10">No hay noticias de artes visuales en este momento.</p>`;
                    return;
                }
                
                querySnapshot.forEach((doc) => {
                    featuredNewsContainer.innerHTML += createNewsCard(doc.data());
                });
            } catch (error) {
                console.error("Error al obtener las noticias de arte: ", error);
                featuredNewsLoader.style.display = 'none';
                // Si falta el índice de Firebase, mostramos un mensaje sutil
                featuredNewsContainer.innerHTML = `<p class="col-span-full text-center text-red-500 p-4 bg-red-100 rounded-lg font-bold">Es posible que necesites crear un índice en Firestore para las noticias. Presiona F12 y revisa la consola para ver el enlace.</p>`;
            }
        }

        // Ejecutar funciones
        fetchAndDisplayFeaturedArtEvents();
        fetchAndDisplayFeaturedArtNews();
    </script>
