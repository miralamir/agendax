...
<head>
    ...
    <title>Agenda de Teatro - BAMARTE</title>
...
<body class="text-gray-800">
    ...
    <div class="container mx-auto px-4">
        ...
        <!-- Header y Navegación (Igual al de la home) -->
        <header class="py-6">
        ...
        <!-- Contenido Principal de la Página de Teatro -->
        <main class="py-16">
            <div class="text-center mb-12">
                <h1 class="text-4xl md:text-5xl font-black text-pink-500">Agenda de Teatro</h1>
                <p class="text-lg text-gray-600 mt-2">Todas las obras y eventos teatrales.</p>
            </div>
    ...
    <script type="module">
        ...
        async function fetchAndDisplayTeatroEvents() {
            try {
                ...
                // 1. Filtrar y ordenar con SEGURIDAD EXTREMA (A prueba de balas)
                const upcomingEvents = events
                    .filter(event => {
                        // Verificamos que el campo exista Y que sea una fecha válida de Firebase
                        ...
                        return true; // Si no tiene fechas fijas (ej. recurrente), lo mostramos
                    })
                    .sort((a, b) => {
                ...
                if (upcomingEvents.length === 0) {
                    eventsContainer.innerHTML = `<p class="col-span-full text-center text-gray-500 font-bold uppercase tracking-widest py-10">No hay eventos próximos en esta categoría.</p>`;
                    return;
                }
                
                // 2. Mostrar las tarjetas (UN SOLO BUCLE)
                    ...
                    const eventCard = `
                        <a href="event-detail.html?id=${event.id}" class="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 flex flex-col group">
                            <div class="relative">
                                ...
                                <div class="absolute top-2 right-2 bg-white bg-opacity-90 text-purple-700 font-black text-xs px-3 py-1 rounded-full shadow-sm uppercase tracking-widest">${dateString}</div>
                            </div>
                            <div class="p-6 flex flex-col flex-grow">
                                <h4 class="text-xl font-black mb-2 uppercase group-hover:text-purple-600 transition-colors leading-tight">${event.title}</h4>
                ...
                console.error("Error al obtener los eventos de teatro: ", error);
                loaderContainer.style.display = 'none';
                eventsContainer.innerHTML = `<p class="col-span-full text-center text-red-500 p-4 bg-red-100 rounded-lg">Ocurrió un error al cargar los eventos. Es posible que necesites crear un índice en Firestore. Revisa la consola del navegador para ver el enlace de creación.</p>`;
            }
        }

        fetchAndDisplayTeatroEvents();
    </script>
